...
<div class="card mb-4">
    <div class="card-body">
        ...
        <form class="row g-3" method="post" action="{{ route('bysitesreports') }}">
            @csrf
            @if($canFilterBySbu)
                <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <select id="networkId" class="form-select" name="network" onchange="getSites()">
                    ...
                    @foreach ($networks as $network)
                        <option value="{{ $network->id }}" {{ (string) request('network') === (string) $network->id ? 'selected' : '' }}>
                            {{ $network->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-12 d-flex gap-2 flex-wrap">
                ...
                <a href="{{ route('bysitesreports') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
</div>
        ...
        <div class="card h-100">
            <div class="card-body">
                ...
                <div class="glass-note mb-3">
                    ...
                    {{ request('startdate') ?: 'Not specified' }} to {{ request('enddate') ?: 'Not specified' }}
                </div>
                <div class="glass-note mb-3">
                    <strong>Selected Network</strong><br>
                    {{ optional($networks->firstWhere('id', request('network')))->name ?: 'All available networks' }}
                </div>
                <div class="glass-note mb-3">
                    <strong>Selected Sites</strong><br>
                    {{ count($selectedSites) ? number_format(count($selectedSites)) . ' site(s) explicitly selected' : 'All matching sites in scope' }}
                </div>
                <div class="glass-note">
                    <strong>Latest Submission</strong><br>
                    {{ $latestSubmission ? \Carbon\Carbon::parse($latestSubmission->created_at)->format('d M Y H:i') : 'No submissions found' }}
                </div>
            </div>
        </div>
    </div>
                ...
                <table class="table table-striped datatable" id="usdsitesTable">
                    ...
                    <tbody>
                        @foreach ($transactions as $transaction)
                            <tr>
                                ...
                                <td>${{ number_format((float) $transaction->usd_mpos, 2) }}</td>
                                <td>${{ number_format((float) $transaction->usd_cash_in_hand_balance, 2) }}</td>
                                <td>{{ $transaction->bank }}</td>
                                <td>${{ number_format((float) $transaction->usd_total_deposited, 2) }}</td>
                                <td>${{ number_format((float) $transaction->usd_debit_sales, 2) }}</td>
                                <td>${{ number_format((float) $transaction->usd_credit_sales, 2) }}</td>
                                <td>{{ data_get($transaction, 'site.POS', 'N/A') }}</td>
                                <td>{{ data_get($transaction, 'site.bank', 'N/A') }}</td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th>${{ number_format((float) $transactions->sum('insurance_transactions'), 2) }}</th>
                            <th>{{ number_format((float) $transactions->sum('zinara_fees'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('third_party_premiums'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('full_cover_premiums'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('other_insurances_usd'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_cash'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_swipe'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_transfers'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_mpos'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_cash_in_hand_balance'), 2) }}</th>
                            <th></th>
                            <th>${{ number_format((float) $transactions->sum('usd_total_deposited'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_debit_sales'), 2) }}</th>
                            <th>${{ number_format((float) $transactions->sum('usd_credit_sales'), 2) }}</th>
                            <th></th>
                            <th></th>
                            <th></th>
                        </tr>
            ...
            <div class="table-responsive table-shell">
                <table class="table table-striped datatable" id="zwgsitesTable">
                    ...
                    <tbody>
                        @foreach ($transactions as $transaction)
                            <tr>
                                ...
                                <td>{{ $transaction->username }}</td>
                                <td>{{ \Carbon\Carbon::parse($transaction->created_at)->format('d M Y H:i') }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_insurance_transactions, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) ($transaction->zwg_zinara_fees ?? 0), 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_third_party_premiums, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_full_cover_premiums, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->other_insurances_zwg, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_swipe, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) ($transaction->zwg_transfers ?? 0), 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) ($transaction->zwg_cash ?? 0), 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_mpos, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_cash_in_hand, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_cash_in_hand_balance, 2) }}</td>
                                <td>{{ $transaction->bank }}</td>
                                <td class="zwg-number">{{ number_format((float) ($transaction->zwg_total_deposited ?? $transaction->zwg_cash_deposited), 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_debit_sales, 2) }}</td>
                                <td class="zwg-number">{{ number_format((float) $transaction->zwg_credit_sales, 2) }}</td>
                                <td>{{ data_get($transaction, 'site.POS', 'N/A') }}</td>
                                <td>{{ data_get($transaction, 'site.bank', 'N/A') }}</td>
                                <td>{{ $transaction->comments }}</td>
                            </tr>
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="4">Totals</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_insurance_transactions'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_zinara_fees'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_third_party_premiums'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_full_cover_premiums'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('other_insurances_zwg'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_swipe'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_transfers'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_cash'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_mpos'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_cash_in_hand'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_cash_in_hand_balance'), 2) }}</th>
                            <th></th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_total_deposited'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_debit_sales'), 2) }}</th>
                            <th class="zwg-number">{{ number_format((float) $transactions->sum('zwg_credit_sales'), 2) }}</th>
                            <th></th>
                            <th></th>
                            <th></th>
                        </tr>
...
<script>
const selectedSiteIds = @json($selectedSites);
...
async function getSites() {
    ...
    try {
        const response = await fetch(`{{ url('/getsites') }}/${networkId}${sbu ? `?sbu=${encodeURIComponent(sbu)}` : ''}`);
        if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.querySelector('#siteVolumeChart')) {
        new Chart(document.querySelector('#siteVolumeChart'), {
            type: 'bar',
            data: {
                labels: @json($topSites->pluck('site_name')->toArray()),
                datasets: [
                    {
                        label: 'USD Transactions',
                        data: @json($topSites->pluck('usd')->toArray()),
                        backgroundColor: 'rgba(15, 107, 110, 0.82)',
                        borderRadius: 12
                    },
                    {
                        label: 'ZWG Transactions',
                        data: @json($topSites->pluck('zwg')->toArray()),

...
<div class="card">
  ...
  @if (session('status'))
        ...
        @endif
    <form method="POST" id="collectionForm" action="{{ route('postcollection') }}">
      @csrf
      <div class="row">
...
<div class="col-lg-2">
      ...
      @if($isCourierClerk)
        <div class="card border-0 shadow-sm mb-4">
          ...
          <div class="card-body">
            @if($courierFaceValueBatches->isEmpty())
            ...
            @else
              ...
              <div class="table-responsive">
                <table class="table table-bordered align-middle">
                  ...
                  <tbody>
                    @foreach($courierFaceValueBatches as $batch)
                      @php($entryKey = (string) $batch->id)
                      <tr>
                        <td>#{{ $batch->batch_id }}</td>
                        <td>{{ $batch->starting }} - {{ $batch->ending }}</td>
                        <td>{{ number_format((float) $batch->batch_balance, 0) }}</td>
                        <td>
                          <input type="hidden" name="face_value_usage[{{ $entryKey }}][fvid]" value="{{ $batch->id }}">
                          <input type="hidden" name="face_value_usage[{{ $entryKey }}][batch_id]" value="{{ $batch->batch_id }}">
                          <input type="number" min="0" step="1" class="form-control courier-usage-field" data-balance="{{ (int) round((float) $batch->batch_balance) }}" data-batch="{{ $batch->batch_id }}" name="face_value_usage[{{ $entryKey }}][used]" value="{{ old("face_value_usage.$entryKey.used", 0) }}">
                        </td>
                        <td>
                          <input type="number" min="0" step="1" class="form-control courier-usage-field" data-balance="{{ (int) round((float) $batch->batch_balance) }}" data-batch="{{ $batch->batch_id }}" name="face_value_usage[{{ $entryKey }}][spoiled]" value="{{ old("face_value_usage.$entryKey.spoiled", 0) }}">
                        </td>
                        <td>
                          <input type="text" class="form-control" name="face_value_usage[{{ $entryKey }}][comments]" value="{{ old("face_value_usage.$entryKey.comments") }}" placeholder="Optional note for reconciliation">
                        </td>
                      </tr>
                    @endforeach
                  </tbody>

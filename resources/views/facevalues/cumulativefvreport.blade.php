...
<div class="card mb-4">
    <div class="card-body">
        ...
        <form method="GET" action="{{ route('cumulativefvreport') }}" class="row g-3">
            @if($showSbuFilter)
                <div class="col-md-3">
            ...
            <div class="col-12 d-flex gap-2 flex-wrap">
                ...
                <a href="{{ route('cumulativefvreport') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
</div>
...
@if($docs->isEmpty())
    <div class="empty-state">
        No cumulative face value activity was found between {{ $startDate->toDateString() }} and {{ $endDate->toDateString() }} in the selected scope.
    </div>
@else
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
                <div>
                    ...
                    <div class="muted">{{ $startDate->toFormattedDateString() }} to {{ $endDate->toFormattedDateString() }} cumulative face value movement.</div>
                </div>
                ...
                <table class="table table-striped datatable" id="cumulativeFaceValuesTable">
                    ...
                    <tbody>
                        @foreach($docs as $data)
                            ...
                            <tr class="{{ $status['row_class'] }}">
                                ...
                                <td>{{ $data['end_date'] }}</td>
                                <td><span class="{{ $openingClass($data['opening_balance']) }}">{{ number_format($data['opening_balance'], 2) }}</span></td>
                                <td><span class="{{ $receivedClass($data['total_received']) }}">{{ number_format($data['total_received'], 2) }}</span></td>
                                <td><span class="{{ $usedClass($data['total_used']) }}">{{ number_format($data['total_used'], 2) }}</span></td>
                                <td><span class="{{ $spoiledClass($data['total_spoiled']) }}">{{ number_format($data['total_spoiled'], 2) }}</span></td>
                                <td><span class="{{ $status['class'] }}">{{ number_format($data['closing_balance'], 2) }}</span></td>
                                <td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th><span class="{{ $openingClass($docs->sum('opening_balance')) }}">{{ number_format($docs->sum('opening_balance'), 2) }}</span></th>
                            <th><span class="{{ $receivedClass($docs->sum('total_received')) }}">{{ number_format($docs->sum('total_received'), 2) }}</span></th>
                            <th><span class="{{ $usedClass($docs->sum('total_used')) }}">{{ number_format($docs->sum('total_used'), 2) }}</span></th>
                            <th><span class="{{ $spoiledClass($docs->sum('total_spoiled')) }}">{{ number_format($docs->sum('total_spoiled'), 2) }}</span></th>
                            <th>
                                @php($totalStatus = $closingMeta($docs->sum('closing_balance'), $docs->sum('total_spoiled')))
                                <span class="{{ $totalStatus['class'] }}">{{ number_format($docs->sum('closing_balance'), 2) }}</span>
                            </th>
                            <th></th>
                        </tr>
                    </tfoot>

...
<?php
    $isCourierClerk = $isCourierClerk ?? false;
    $courierFaceValueBatches = $courierFaceValueBatches ?? collect();
?>
...
<div class="card">
  ...
  <?php if($isCourierClerk): ?>
            <div class="alert alert-info">
                Courier Connect sales should be linked to the issued face value batch or batches used for the day so reporting and reconciliation stay aligned.
            </div>
        <?php endif; ?>
      ...
      <?php if($isCourierClerk): ?>
        <div class="card border-0 shadow-sm mb-4">
          <div class="card-header bg-light">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
              <span>Courier Connect Face Value Linkage</span>
              <span class="soft-chip"><i class="bi bi-upc-scan"></i> Issued document usage</span>
            </div>
          </div>
          <div class="card-body">
            <?php if($courierFaceValueBatches->isEmpty()): ?>
              <div class="alert alert-warning mb-0">
                No active issued face value batches were found for your account. Courier sales with collections should only be submitted once stock has been issued to you.
              </div>
            <?php else: ?>
              <p class="text-muted mb-3">Enter the used and spoiled face values against the exact batches worked today. Any row with activity will be posted together with this daily collection.</p>
              <div class="table-responsive">
                <table class="table table-bordered align-middle">
                  ...
                  <tbody>
                    <?php $__currentLoopData = $courierFaceValueBatches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $batch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                      <?php ($entryKey = (string) $batch->id); ?>
                      <tr>
                        <td>#<?php echo e($batch->batch_id); ?></td>
                        <td><?php echo e($batch->starting); ?> - <?php echo e($batch->ending); ?></td>
                        <td><?php echo e(number_format((float) $batch->batch_balance, 0)); ?></td>
                        <td>
                          <input type="hidden" name="face_value_usage[<?php echo e($entryKey); ?>][fvid]" value="<?php echo e($batch->id); ?>">
                          <input type="hidden" name="face_value_usage[<?php echo e($entryKey); ?>][batch_id]" value="<?php echo e($batch->batch_id); ?>">
                          <input type="number" min="0" step="1" class="form-control courier-usage-field" data-balance="<?php echo e((int) round((float) $batch->batch_balance)); ?>" data-batch="<?php echo e($batch->batch_id); ?>" name="face_value_usage[<?php echo e($entryKey); ?>][used]" value="<?php echo e(old("face_value_usage.$entryKey.used", 0)); ?>">
                        </td>
                        <td>
                          <input type="number" min="0" step="1" class="form-control courier-usage-field" data-balance="<?php echo e((int) round((float) $batch->batch_balance)); ?>" data-batch="<?php echo e($batch->batch_id); ?>" name="face_value_usage[<?php echo e($entryKey); ?>][spoiled]" value="<?php echo e(old("face_value_usage.$entryKey.spoiled", 0)); ?>">
                        </td>
                        <td>
                          <input type="text" class="form-control" name="face_value_usage[<?php echo e($entryKey); ?>][comments]" value="<?php echo e(old("face_value_usage.$entryKey.comments")); ?>" placeholder="Optional note for reconciliation">
                        </td>
                      </tr>
              ...
              <div id="courier-usage-error" class="text-danger small mt-2"></div>
            <?php endif; ?>
          </div>
        </div>
      <?php endif; ?>
...
<script>
document.addEventListener("DOMContentLoaded", function() {
    const usageFields = Array.from(document.querySelectorAll('.courier-usage-field'));
    const usageError = document.getElementById('courier-usage-error');
    const submitBtn = document.getElementById('submit-button');

    if (!usageFields.length || !usageError || !submitBtn) {
        return;
    }

    function validateCourierUsage() {
        const rows = new Map();
        let hasError = false;
        let errorMessage = '';
        ...
        usageFields.forEach((field) => {
            ...
            if (!rows.has(row)) {
                rows.set(row, {
                    balance: parseInt(field.dataset.balance || '0', 10),
                    batch: field.dataset.batch || '',
                    total: 0,
                });
            }

            const state = rows.get(row);
            state.total += parseInt(field.value || '0', 10) || 0;
        });

        rows.forEach((state) => {
            if (state.total > state.balance) {
                hasError = true;
                errorMessage = 'Courier face value usage cannot be greater than the available balance for batch #' + state.batch + '.';
            }
        });

        usageError.textContent = errorMessage;
        submitBtn.disabled = hasError;
    }

    usageFields.forEach((field) => {
        field.addEventListener('input', validateCourierUsage);
    });

    validateCourierUsage();
});
</script>
...
<?php echo $__env->make('layouts.main', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/macbookair/Documents/Projects/gruma/resources/views/collection/create.blade.php ENDPATH**/ ?>